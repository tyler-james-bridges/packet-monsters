...
page.on('console', m => console.log(`C[${m.type()}]:`, m.text().slice(0, 2000)));
await page.goto(`${base}?seed=vault-alpha&shot=idle`, { waitUntil: 'domcontentloaded', timeout: 300000 });
await page.waitForFunction(() => Boolean(window.__harness), null, { timeout: 300000 });
for (let i = 0; i < 8; i++) {
  await new Promise(r => setTimeout(r, 5000));
  const s = await page.evaluate(() => new Promise((res) => {
    let ticks = 0;
    const t0 = performance.now();
    const tick = () => { ticks++; if (performance.now() - t0 < 1000) requestAnimationFrame(tick); else res({
      ready: window.__harness?.ready, shots: window.__harness?.shots, rafPerSec: ticks,
    }); };
